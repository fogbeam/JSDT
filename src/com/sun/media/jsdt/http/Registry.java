...
/**
 * JSDT Registry class (HTTP implementation).
 *
...
Registry extends JSDTObject
         implements AbstractRegistry, Runnable, httpDebugFlags {
     ...
     */
    private HttpThread proxyThread = null;
    ...
    private Hashtable<Integer, ServerListenerId> listenerIds = null;
    ...
    private final Hashtable<Integer, HttpThread> permThreads;


/**
 * <A NAME="SD_REGISTRY"></A>
 * <EM>Registry</EM> the constructor for the JSDT HTTP based registry.
 */
    ...
    Registry() {
        ...
        try {
            factory = (JSDTSocketFactory)
                Util.getClassForName(httpFactoryClass).newInstance();
        } catch (Exception e) {
 ...
 */
    ...
    addListener(Message message) {
        ...
        if ((listenerId = listenerIds.get(message.id)) == null) {
            listenerId = new ServerListenerId(this.toString());
            putBack = true;
        }
 ...
 */
    ...
    private void
    informListeners(JSDTThread thread, String clientName,
        ...
        for (Enumeration k = listenerIds.keys(); k.hasMoreElements();) {
            ...
            try {
                thread.writeMessageHeader(thread.dataOut, (short) 1, id,
                                  T_Registry, T_InformListener, false, false);
 ...
 */
    ...
    getMessage(Message message) {
        ...
        try {
            ...
            if (nextMessage != null) {
                t.dataOut.writeInt(nextMessage.length);
                t.dataOut.write(nextMessage,  0, nextMessage.length);
            } else {
 ...
 */
    ...
    getBufferedMessage(int id) {
        ...
        if (Registry_Debug) {
            debug("Registry: getBufferedMessage:" +
                  " id #: " + id);
        }
 ...
 */
    ...
    putBufferedMessage(int id, byte[] byteArray) {
        ...
        if (Registry_Debug) {
            debug("Registry: putBufferedMessage:" +
                  " id #: "       + id +
 ...
 */
    ...
    permanent(Message message) {
        ...
        if (Registry_Debug) {
            debug("Registry: permanent:" +
                  " message: " + message);
        }

        addPermanentThread(message.id, (HttpThread) message.thread);
    ...
    addPermanentThread(int id, HttpThread thread) {
        if (Registry_Debug) {
            debug("Registry: addPermanentThread:" +
                  " id: "     + id +
                  " thread: " + thread);
        }
    ...
    removePermanentThread(int id) {
        if (Registry_Debug) {
            debug("Registry: removePermanentThread:" +
                  " id: " + id);
        }
 ...
 */
    ...
    sendAsynchMessage(int id, byte[] byteArray) {
        HttpThread thread = permThreads.get(id);

        if (Registry_Debug) {
            debug("Registry: sendAsynchMessage:" +
                  " id #: "       + id +
                  " byte array: " + byteArray);
        }
        ...
        try {
            if (thread == null) {    /* No permanent connection. */
                putBufferedMessage(id, byteArray);
            ...
            error("Registry: sendAsynchMessage: ", ioe);
        }
    }
 ...
 */
    ...
    registryExists(String registryType, int port)
        ...
        if (!registryType.equals("http")) {
            throw new NoRegistryException();
        }

        oldTimeout = timeoutValue;
        if (oldTimeout > 3000) {
        ...
        Connection.setProperty("timeoutPeriod", Long.toString(timeoutValue));
 ...
 */
    ...
    initProxy(String host, int port)
        ...
        proxyThread = new HttpThread(host, port, false);
 ...
 */
    ...
    bind(Message message) {
        ...
        try {
            ...
            informListeners(t, clientName, name, type);
        } catch (IOException e) {
 ...
 */
    ...
    getSessionNo(Message message) {
        ...
        try {
            ...
            t.dataOut.writeInt(0);      // return value.
 ...
 */
    ...
    lookup(Message message) {
        ...
        try {
            ...
            t.dataOut.writeInt(retval);
            if (retval == 0) {
                Short value = values.get(boundName);

                t.dataOut.writeShort(value);
            }
            t.flush();
            t.finishMessage();
        } catch (IOException e) {
    ...
    handleRequests() {
        ...
        try {
            while (true) {
                ...
                Util.startThread(rst, "RegistryServerThread:" + sock, true);
            }
        } catch (Exception e) {
