 ...
 */
    ...
    parseServerByteArrayMessage(Message message) {
        ...
        if (message.action == T_Authenticate) {
            getAuthenticationResult(message);
            return;
        }
...
/* The server will send back a reply to the T_Permanent message sent by the
 * SessionPermThread. Just ignore it.
 */
                ...
                sp.proxyThread.havePermanent = true;
                message.thread.finishReply();
                break;
            case T_Close:                   /* CLOSE. */

/* Depending upon where the HttpThread: run() method currently was when the
 * HttpThread: terminate() method, was called, it's possible that we might
 ...
 */
    ...
    parseServerTokenMessage(Message message) {
        ...
        if (token == null) {
            error("impl.cannot.find", tokenName);
        } else {
            switch (message.action) {
                case T_InformListener:               /* INFORMLISTENER. */
