...
/**
 * JSDT Naming proxy class (HTTP implementation).
 *
...
NamingProxy extends JSDTObject
            implements AbstractNamingProxy, httpDebugFlags, Runnable {
    ...
    HttpThread proxyThread = null;

    // The actual thread.
    private Thread NPThread;

    // Proxy thread that is permanently connected to the registry.
    private HttpThread permThread;

    // The thread handle to the permanent thread connection.
 ...
 */
    ...
    initProxy(Hashtable connections, String host, int port)
        ...
        try {
            ...
            if (!proxyThread.mustPing()) {
                permThread  = new NamingPermThread(this, host, port,
                                                   proxyThread.getId());
            }
        ...
        if (!proxyThread.mustPing()) {
            pThread = Util.startThread(permThread,
                                "NamingPermThread:" + host + ":" + port, true);
        }

        thread = Util.startThread(this, "NamingProxy", true);
        NPThread = Util.startThread(proxyThread,
 ...
 */
    ...
    cleanupClient(String name, Client client, int id)
        ...
        if (client instanceof httpClient) {
            ...
            if (hc.cc != null) {
                HttpThread thread = hc.cc.proxyThread;
 ...
 *
 * where valid impl types are "socket", "multicast" and "http".
 * where valid object types are "Session" and "Client".
 *
 * So for example:
 *     bind("jsdt://stard:3355/http/Session/chatSession", chatSession);
 *
 *     bind("jsdt://stard:4386/http/Client/fredClient", fredClient);
 *
            ...
            while (c != null) {
                if (c == Util.getClassForName(className)) {
                    if (Util.isSession(urlString)) {
                        ...
                        ((httpClient) object)._createServer(objectName,
                                                            name, port);
                    } else {
 ...
 */
    ...
    private void
    setupSessionProxy(httpSession session, URLString urlString, String name,
        ...
        try {
            ...
            t.writeMessageHeader(t.dataOut, sessionNo, t.getId(),
                                 SessionImpl.M_Session, T_CreateSession,
                                 true, true);
 ...
 *
 * So for example:
 *   Session session = lookup("jsdt://stard:3355/http/Session/chatSession");
 *
 *   Client  client  = lookup("jsdt://stard:4386/http/Client/fredClient");
 *
 * @param urlString the URLString object for the given name.
 *
 * @exception ConnectionException if a connection error occured.
        ...
        try {
            if (Util.isSession(urlString)) {
                ...
                synchronized (sessions) {
                    ...
                    session = (httpSession) sessions.get(url);
                    if (session == null) {
                        String className = "com.sun.media.jsdt." + connectType +
                                           "." + connectType + "Session";
 ...
 */
        ...
        if (proxyThread.mustPing()) {
            ((NamingProxyThread) proxyThread).startRegistryPingThread(listener);
        }
 ...
 */
    ...
    removeRegistryListener(RegistryListener listener)
        ...
        if (proxyThread.mustPing()) {
            ((NamingProxyThread) proxyThread).stopRegistryPingThread(listener);
        }
 ...
 */
    ...
    authenticateClient(URLString urlString, int action, Client client)
        ...
        try {
            proxyThread.waitForPermanentConnection();

            proxyThread.writeMessageHeader(proxyThread.dataOut, (short) 1, id,
                                           T_Registry, T_Authenticate,
                                           true, true);
