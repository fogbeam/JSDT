    ...
    SessionProxy(SessionImpl session, String url, String name,
        ...
        try {
            ...
            if (TCPSocketServer.socketServers != null &&
                ...
                ((SameVMThread) proxyThread).setReplyThread(sessionNo,
                                           serverThread.getId(), serverThread);

                serverThread.setReplyThread(sessionNo, proxyThread.getId(),
                                            (SameVMThread) proxyThread);
            } else {
 ...
 */
    ...
    initProxy(String name, SessionImpl session, Object object) {
        ...
        if (proxyThread instanceof SessionProxyThread &&
            !proxyThread.mustPing()) {
            try {
                permThread = new SessionPermThread(session, this,
                                                   proxyThread.getAddress(),
                                                   proxyThread.getPort(),
                                                   proxyThread.getId());
            } catch (SocketException | UnknownHostException e) {
                error("SessionProxy: initProxy: ", e);
            }

            pThread = Util.startThread(permThread,
                                       "SessionPermThread:" + name, true);
        }
    }
 ...
 *
 * @param clientName the name of the client attempting to do the priviledged
 * operation.
 ...
 */
    ...
    invite(Client[] clients)
        ...
        if (clients != null) {
            for (int i = 0; i < clients.length; i++) {
                if (clients[i] instanceof httpClient) {
                    httpClient client = (httpClient) clients[i];
                    HttpThread thread = client.cc.proxyThread;
 ...
 */
    ...
    removeSession(String sessionName) {
        ...
        if (session != null) {
            ...
            ((httpSession) session).namingProxy.cleanupSession(
 ...
 */
    ...
    close(boolean closeConnection)
        ...
        namingProxy = ((httpSession) session).namingProxy;
