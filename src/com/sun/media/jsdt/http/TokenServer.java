 ...
 */
    ...
    give(Message message) {
        ...
        try {
            ...
            receivingId         = getIdForClient(receivingClientName);
            receivingClient     = getClientByName(receivingClientName);
            givingClient        = getClientByName(givingClientName);
            givingId            = getIdForClient(givingClientName);
        } catch (IOException e) {
            error("TokenServer: give: ", e);
        }

        if (givingClient == null || receivingClient == null ||
            givingId == 0 || receivingId == 0) {
            retval = JSDTException.NO_SUCH_CLIENT;
        } else if (givingId != message.id) {
        ...
        try {
            if (retval == 0) {
                ...
                message.thread.writeMessageHeader(message.thread.dataOut,
                              message.sessionNo, receivingId,
                              ClientImpl.M_Client, T_TokenGiven, false, false);
                message.thread.dataOut.writeUTF(name);
                message.thread.dataOut.writeUTF(receivingClientName);
        ...
        if (retval == 0) {
            informListeners(message.thread, session.getName(),
                            givingClientName, name,
                            TokenEvent.GIVEN, message.type);
        }
    }
    ...
    grab(Message message) {
        ...
        clientId = (Integer) ((ClientImpl) client).getCheck();
        ...
        if (retval == 0) {
            if (sendReleaseEvent) {
                informListeners(message.thread, session.getName(),
                                givingClient.getName(), name,
                                TokenEvent.RELEASED, message.type);
            }
            informListeners(message.thread, session.getName(),
                            client.getName(), name, eventType, message.type);
        }
    }
 ...
 */
    ...
    join(Message message) {
        ...
        if (retval == 0) {
            informListeners(message.thread, session.getName(), clientName,
 ...
 */
    ...
    leave(Message message, String clientName, boolean isCleanup) {
        ...
        super.leave(message, session.getName(),
                    TokenImpl.M_Token, clientName, isCleanup);
    }
