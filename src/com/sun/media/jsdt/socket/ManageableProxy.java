...
/**
 * JSDT client-side proxy parent class (socket implementation).
 *
...
ManageableProxy extends JSDTObject
                implements AbstractManageableProxy, socketDebugFlags {

    // The name of this proxy object.
    protected String name = null;

    // Proxy thread for this object.
    SocketThread proxyThread;
    ...
    protected SocketThread managerThread;
    ...
    isManaged(char objectType, String objectName)
        ...
        try {
            sp.proxyThread.writeMessageHeader(sp.proxyThread.dataOut,
                        sessionNo, id, objectType, T_IsManaged, true, true);
 ...
 */
    ...
    addListener(EventListener listener, char listenerType)
        ...
        try {
            sp.proxyThread.writeMessageHeader(sp.proxyThread.dataOut,
                       sessionNo, id, listenerType, T_AddListener, true, true);
            sp.proxyThread.dataOut.writeUTF(name);
 ...
 */
    ...
    public void
    attachManager(JSDTManager manager, char managerType, Manageable manageable)
        ...
        Message         message;
        int             retval;
        SocketThread    managerThread = null;
        SocketThread    thread        = sp.proxyThread;
        int             id            = thread.getId();
        short           sessionNo     = sp.getSessionNo();
        ...
        try {
            ...
            if (TCPSocketServer.socketServers != null &&
                ...
                ((SameVMThread) managerThread).setReplyThread(
                                                managerServerThread);

                managerServerThread.setReplyThread(
                                                (SameVMThread) managerThread);
            } else {
        ...
        try {
            managerThread.writeMessageHeader(managerThread.dataOut,
                           sessionNo, id, managerType, T__Manager, true, true);
 ...
 */
    ...
    expelInvite(char messageType, Client[] clients, char objectType)
        ...
        try {
            sp.proxyThread.writeMessageHeader(sp.proxyThread.dataOut,
                           sessionNo, id, objectType, messageType, true, true);
            sp.proxyThread.dataOut.writeUTF(name);
 ...
 */
    ...
    final synchronized SocketThread
    getManagerThread() {
        if (ManageableProxy_Debug) {
    ...
    removeListener(EventListener listener, char listenerType)
        ...
        try {
            sp.proxyThread.writeMessageHeader(sp.proxyThread.dataOut,
                   sessionNo, id, listenerType, T_RemoveListener, true, true);
            sp.proxyThread.dataOut.writeUTF(name);
 ...
 */
    ...
    private synchronized void
    setManagerThread(SocketThread thread) {
        if (ManageableProxy_Debug) {
